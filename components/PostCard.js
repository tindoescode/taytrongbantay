    ...
    <div className="max-w-md mx-auto my-2 bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
      <div className="md:flex items-center p-2">
        ...
        <div className="relative h-48 w-full md:w-48 md:flex-shrink-0 m-2 rounded ring-1 ring-green-200">
          <Image layout="fill" objectFit="cover" src={thumbnail} alt="thumbnail text" />
        </div>
        </a>
        </Link>
        <div className="p-8">
          ...
          <Link href={href}>
            <a className="block mt-1 text-lg leading-tight font-medium text-black">{title || <Skeleton /> }</a>
          </Link>
          <p className="mt-2 text-gray-500 text-xs line-clamp-5">{description || <Skeleton />}</p>
        </div>
      </div>
    </div>
  )
